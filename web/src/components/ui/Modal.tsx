  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
      ...
      <div className={cn('relative z-50 w-full rounded-2xl bg-[var(--card)] shadow-glow-lg border border-[var(--border)]/60 animate-scale-in', sizes[size])}>
        {title && (
          <div className="flex items-center justify-between border-b border-[var(--border)]/40 px-6 py-4 bg-gradient-to-r from-[var(--muted)]/30 to-transparent">
            <h2 className="text-lg font-bold text-gradient">{title}</h2>
            <button onClick={onClose} className="rounded-lg p-1.5 text-[var(--muted-foreground)] hover:bg-[var(--muted)] hover:shadow-soft transition-all duration-200 hover:scale-110">
