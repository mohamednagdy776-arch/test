  ...
  return (
    <div className="flex gap-4 h-[calc(100dvh-8rem)]">
      ...
      <div className={`${activeMatch ? 'block' : 'hidden lg:block'} flex-1 min-w-0`}>
        {deepLinkError ? (
          <div className="flex h-full items-center justify-center rounded-2xl bg-gradient-to-br from-[#ECFDF5] to-[#F0FDF4] border border-emerald-100 shadow-lg shadow-emerald-500/10">
            <div className="text-center p-8">
              <p className="text-4xl mb-3">⚠️</p>
              <p className="text-sm font-medium text-[#10B981] mb-4">{deepLinkError}</p>
              <button
                onClick={() => { setDeepLinkError(null); handledRef.current = null; }}
                className="rounded-xl bg-gradient-to-r from-emerald-500 to-emerald-600 px-5 py-2 text-sm font-semibold text-white shadow-lg shadow-emerald-500/25 hover:shadow-xl transition-all"
              >
                إعادة المحاولة
              </button>
            </div>
          ...
          <div className="flex h-full items-center justify-center rounded-2xl bg-gradient-to-br from-[#ECFDF5] to-[#F0FDF4] border border-emerald-100">
            <div className="text-center">
              ...
              <p className="text-sm text-[#10B981]">جاري فتح المحادثة...</p>
            </div>
          </div>
        ) : activeMatch ? (
