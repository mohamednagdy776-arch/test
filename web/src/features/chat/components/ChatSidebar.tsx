...
export const ChatSidebar = ({ activeConversationId, onSelect, onCreateGroup }: Props) => {
  ...
  return (
    <div className="w-72 shrink-0 border-l flex flex-col bg-[var(--muted)]">
      <div className="p-4 border-b bg-[var(--card)]">
        <h2 className="font-bold text-[var(--foreground)] mb-3">المحادثات</h2>
        <input
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          placeholder="🔍 بحث في المحادثات..."
          className="w-full rounded-lg border border-[var(--border)] px-3 py-2 text-sm focus:border-primary focus:outline-none"
        />
      </div>

      <div className="flex border-b bg-[var(--card)]">
        {[
          ...
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`flex-1 py-2.5 text-xs font-medium transition-colors ${
              tab === t.key ? 'border-b-2 border-primary text-primary' : 'text-[var(--muted-foreground)]'
            }`}>
            {t.label}
          </button>
        ))}
        ...
        {tab === 'chats' ? (
          convLoading ? (
            <div className="p-4 space-y-2">
              {[1,2,3].map((i) => <div key={i} className="h-14 rounded-lg bg-[var(--muted)] animate-pulse" />)}
            </div>
            ...
            filteredConvos.map((conv) => (
              <button key={conv.id} onClick={() => onSelect({ id: conv.id, user1Id: '', user2Id: '', score: 0, status: 'chat', createdAt: conv.createdAt } as Match)}
                className={`w-full flex items-center gap-3 px-4 py-3 text-right hover:bg-[var(--card)] transition-colors border-b border-[var(--border)] ${
          ...
          <div>
            <p className="px-4 py-2 text-xs text-[var(--muted-foreground)] bg-yellow-50 border-b border-yellow-100">
            ...
            {usersLoading ? (
              ...
              (usersData?.data ?? []).map((u: any) => {
                ...
                return (
                  <div key={u.id} className="flex items-center gap-3 px-4 py-3 border-b border-[var(--border)]">
                    <div className="h-10 w-10 shrink-0 rounded-full bg-[var(--muted)] flex items-center justify-center text-[var(--muted-foreground)] font-bold text-sm">
                      {u.email?.charAt(0).toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-[var(--foreground)] truncate">{u.email}</p>
                      <p className="text-xs text-[var(--muted-foreground)] capitalize">{u.accountType}</p>
                    </div>
                    {existingMatch ? (
