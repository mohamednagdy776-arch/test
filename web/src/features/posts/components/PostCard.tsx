...
function ReactionPicker({ onSelect, onClose }: { onSelect: (type: string) => void; onClose: () => void }) {
  return (
    <div className="absolute bottom-full mb-2 left-0 bg-[#FDFAF5] rounded-2xl shadow-lg border border-[#C8D8DF]/60 p-3 animate-scale-in z-20">
  ...
  return (
    <div className="relative" ref={pickerRef}>
      <div className="flex items-center gap-1">
        ...
        {total > 0 && <span className="text-xs text-[#547792] font-medium">{total}</span>}
      </div>
...
function ShareModal({ isOpen, onClose, postId, postContent }: { isOpen: boolean; onClose: () => void; postId: string; postContent: string }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center animate-fade-in">
      ...
      <div className="relative w-full max-w-md mx-4 bg-[#FDFAF5] rounded-2xl p-6 animate-scale-in">
        ...
        <textarea
          ...
          className="w-full rounded-xl border border-[#C8D8DF] bg-[#EAE0CF]/40 px-4 py-3 text-sm text-[#131F2E] placeholder:text-[#BFB9AD] focus:outline-none focus:ring-2 focus:ring-[#547792]/20 focus:border-[#547792] mb-4"
          rows={3}
        />
        <div className="bg-[#EAE0CF]/40 rounded-xl p-3 mb-4">
        ...
        <button
          ...
          className="w-full py-3 rounded-xl text-[#FDFAF5] font-medium hover:shadow-md disabled:opacity-40 transition-all"
...
function PostMenu({ postId, post, onClose }: { postId: string; post: any; onClose: () => void }) {
  ...
  return (
    <div className="absolute right-0 top-full mt-2 w-56 bg-[#FDFAF5] rounded-xl shadow-lg border border-[#C8D8DF]/60 py-2 animate-scale-in z-30">
      {menuItems.map((item, i) => (
        <button
          key={i}
          onClick={() => { item.action(); onClose(); }}
          className={cn('w-full px-4 py-2.5 text-right text-sm hover:bg-[#EAE0CF]/50 flex items-center gap-3', item.danger ? 'text-red-500' : 'text-[#213448]')}
        >
          <span>{item.icon}</span>
          {item.label}
        </button>
...
function LinkPreview({ url, title, description, image }: { url: string; title?: string; description?: string; image?: string }) {
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className="block mt-3 rounded-xl overflow-hidden border border-[#C8D8DF]/60 hover:border-[#547792] transition-colors">
...
function PollDisplay({ postId, options }: { postId: string; options: { text: string; votes: number }[] }) {
  ...
  return (
    <div className="mt-3 space-y-2">
      {updatedOptions.map((opt, i) => {
        ...
        return (
          <button
            ...
            disabled={voted !== null}
            className={cn(
              'w-full relative overflow-hidden rounded-lg text-sm text-right transition-all',
              voted === i ? 'ring-2 ring-[#547792]' : '',
      ...
      <p className="text-xs text-[#547792] text-center pt-1">{totalVotes} صوت</p>
    </div>
  );
}
...
export function PostCard({ post, showGroupLink = true }: { post: any; showGroupLink?: boolean }) {
  ...
  return (
    <article className="rounded-2xl bg-[#FDFAF5] shadow-card border border-[#C8D8DF]/60 overflow-hidden transition-all duration-300 hover:shadow-card-hover relative">
      <div className="flex items-center gap-3 p-4 pb-0">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {post.feeling && <span className="text-xs text-[#547792]">شعور {post.feeling}</span>}
        ...
        <div className="relative">
          <button onClick={() => setShowMenu(!showMenu)} className="rounded-lg p-1.5 text-[#547792] hover:bg-[#D4E8EE] transition-colors">
      ...
      {post.bgColor && !mediaUrl ? (
        <div className="px-4 py-6 m-4 rounded-xl text-center" style={{ backgroundColor: post.bgColor }}>
        ...
        <>
          <div className="px-4 py-3">
            ...
            {isShared && post.originalPost && (
              <div className="mt-3 p-3 rounded-xl bg-[#EAE0CF]/40 border border-[#C8D8DF]/40">
                <div className="flex items-center gap-2 mb-2">
                  <div className="h-6 w-6 rounded-full bg-[#547792] text-[#FDFAF5] text-xs flex items-center justify-center">
          ...
          {mediaUrl && (
            <div>
              {post.mediaType === 'video' ? (
                <video src={mediaUrl} controls className="w-full max-h-[480px] object-cover" />
              ) : post.mediaUrls?.length > 1 ? (
                <div className="grid grid-cols-2 gap-1">
                  {post.mediaUrls.map((url: string, i: number) => (
                    <img key={i} src={`${apiUrl}${url}`} alt="" className="w-full h-48 object-cover" />
                  ))}
                </div>
              ) : (
                <img src={mediaUrl} alt="" className="w-full max-h-[480px] object-cover" loading="lazy" />
              )}
            </div>
          )}
        </>
      ...
      <div className="px-4 py-3 space-y-3">
        ...
        <div className="flex items-center gap-4 pt-2 border-t border-[#C8D8DF]/40">
          <button onClick={() => setShowComments(!showComments)} className={cn('flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium transition-all duration-200 flex-1 justify-center', showComments ? 'bg-[#D4E8EE] text-[#213448]' : 'text-[#547792] hover:bg-[#EAE0CF]/50')}>
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8.625 12a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 01-2.555-.337A5.972 5.972 0 015.41 20.97a5.969 5.969 0 01-.474-.065 4.48 4.48 0 00.978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25z" /></svg>
            تعليقات
          </button>
          <button onClick={() => setShowShareModal(true)} className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium text-[#547792] hover:bg-[#EAE0CF]/50 transition-all duration-200 flex-1 justify-center">
