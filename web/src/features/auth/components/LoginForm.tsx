  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <button type="button" onClick={() => window.location.href = `${process.env.NEXT_PUBLIC_API_URL}/auth/oauth/google?redirect_uri=${encodeURIComponent(window.location.origin + '/auth/callback')}`}
        ...
        <button type="button" onClick={() => window.location.href = `${process.env.NEXT_PUBLIC_API_URL}/auth/oauth/apple?redirect_uri=${encodeURIComponent(window.location.origin + '/auth/callback')}`}
          className="flex h-12 items-center justify-center gap-2.5 rounded-2xl border border-[#E5E7EB] bg-white text-sm font-medium text-[#374151] hover:bg-[#F9FAFB] transition-all duration-200">
